...
def extend_with_flipped_entries(roidb, dataset):
    ...
    for entry in roidb:
        ...
        # for list of sub-boxes groups
        if type(boxes) is list:
            for sub_boxes in boxes:
                sub_x1 = sub_boxes[:, 0].copy()
                sub_x2 = sub_boxes[:, 2].copy()
                sub_boxes[:, 0] = width - sub_x2 - 1
                sub_boxes[:, 2] = width - sub_x1 - 1
                assert (sub_boxes[:, 2] >= sub_boxes[:, 0]).all()
        else:
            oldx1 = boxes[:, 0].copy()
            oldx2 = boxes[:, 2].copy()
