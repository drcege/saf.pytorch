...
class JsonDataset(object):
    ...
    def get_roidb(
        ...
        if proposal_file is not None:
            ...
            self.debug_timer.tic()
            if as_is:
                self._replace_proposals_from_file(
    ...
    def _add_proposals_from_file(
        ...
        logger.info('Adding proposals from: {}'.format(proposal_file))
    ...
    def _replace_proposals_from_file(
        self, roidb, proposal_file, min_proposal_size, top_k, crowd_thresh
    ):
        """Load processed proposals from a proposals file to an roidb."""
        logger.info('Loading processed proposals from: {}'.format(proposal_file))
        with open(proposal_file, 'rb') as f:
            proposals = pickle.load(f)
        id_field = 'indexes' if 'indexes' in proposals else 'ids'  # compat fix
        _sort_proposals(proposals, id_field)
