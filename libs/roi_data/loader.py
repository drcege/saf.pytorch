...
class RoiDataLoader(data.Dataset):
    ...
    def __getitem__(self, index_tuple):
        ...
        blobs, break_points = get_minibatch(single_db, self._num_classes)
        ...
        blobs['break_points'] = np.array(break_points, dtype=np.int32)
...
def collate_minibatch(list_of_blobs):
    ...
    """
    assert len(list_of_blobs) == 1
    blob = list_of_blobs[0]
    for k in blob.keys():
        blob[k] = torch.from_numpy(blob[k]).cuda()
    return blob
