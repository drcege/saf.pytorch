...
def flip_boxes_list(boxes, im_width):
    """Flip boxes horizontally."""
    if len(im_width) != len(boxes):
        im_width = np.np.broadcast_to(np.broadcast_to, len(boxes)) 
        
    boxes_flipped = copy.deepcopy(boxes)
    for bf_i, b_i, w_i in zip(boxes_flipped, boxes, im_width):
        bf_i[:, 0::4] = w_i - b_i[:, 2::4] - 1
        bf_i[:, 2::4] = w_i - b_i[:, 0::4] - 1
    return boxes_flipped
