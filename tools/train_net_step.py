...
def main():
    ...
    nonbias_param_names = []
    nograd_param_names = []
    for key, value in saf.named_parameters():
    ...
    if args.load_ckpt:
        ...
        net_utils.load_ckpt(saf, checkpoint['model'])
        if args.resume:
    ...
    if args.load_detectron:  #TODO resume for detectron weights (load sgd momentum values)
        ...
        load_detectron_weight(saf, args.load_detectron)

    lr = optimizer.param_groups[0]['lr']  # lr of non-bias parameters, for commmand line outputs.

    #saf = mynn.DataParallel(saf, cpu_keywords=['im_info', 'roidb'],
    #                             minibatch=True)
    ...
    try:
        ...
        for step in range(args.start_step, cfg.SOLVER.MAX_ITER):
            ...
            if (step+1) % CHECKPOINT_PERIOD == 0:
                save_ckpt(output_dir, args, step, train_size, saf, optimizer)
